...
export interface InvestmentMonth { month:number; stockValueCents:Cents; homeValueCents:Cents; mortgageBalanceCents:Cents; equityCents:Cents; saleProceedsCents:Cents; principalCents:Cents; interestCents:Cents; ownerOutlayCents:Cents; rentCents:Cents; stockContributionCents:Cents }
...
export function validateInvestmentAssumptions(a:InvestmentAssumptions):InvestmentValidationError[]{
  ...
  const moneyFields:(keyof InvestmentAssumptions)[]=["homePriceCents","monthlyRentCents","annualInsuranceCents","monthlyHoaCents"];
  ...
  const rates:(keyof InvestmentAssumptions)[]=["mortgageRateBps","stockReturnBps","homeAppreciationBps","purchaseCostBps","sellingCostBps","rentGrowthBps","propertyTaxBps","insuranceGrowthBps","hoaGrowthBps","maintenanceBps"];
...
function crossovers(months:InvestmentMonth[],home:(m:InvestmentMonth)=>number):InvestmentCrossover[]{
  const found:InvestmentCrossover[]=[]; let previous=Math.sign(months[0].stockValueCents-home(months[0]));
  for(const point of months.slice(1)){const sign=Math.sign(point.stockValueCents-home(point));if(sign!==0&&previous!==0&&sign!==previous)found.push({month:point.month,year:Math.ceil(point.month/12),leader:sign>0?"rent-invest":"buy"});if(sign!==0)previous=sign} return found;
}
export function calculateInvestmentComparison(a:InvestmentAssumptions):InvestmentCalculation{
  ...
  let stock=a.homePriceCents*(a.downPaymentBps+a.purchaseCostBps)/10_000,home=a.homePriceCents,balance=loan;
  const months:InvestmentMonth[]=[{month:0,stockValueCents:rounded(stock),homeValueCents:rounded(home),mortgageBalanceCents:rounded(balance),equityCents:rounded(home-balance),saleProceedsCents:rounded(home*(1-a.sellingCostBps/10_000)-balance),principalCents:0,interestCents:0,ownerOutlayCents:0,rentCents:0,stockContributionCents:0}];
  for(let month=1;month<=a.horizonYears*12;month++){
    stock*=1+stockMonthly; home*=1+homeMonthly;
    ...
    const contribution=owner-rent; stock+=contribution;
    months.push({month,stockValueCents:rounded(stock),homeValueCents:rounded(home),mortgageBalanceCents:rounded(balance),equityCents:rounded(home-balance),saleProceedsCents:rounded(home*(1-a.sellingCostBps/10_000)-balance),principalCents:rounded(principal),interestCents:rounded(interest),ownerOutlayCents:rounded(owner),rentCents:rounded(rent),stockContributionCents:rounded(contribution)});
  }
  const years:InvestmentYear[]=[];
  for(let year=1;year<=a.horizonYears;year++){const slice=months.slice((year-1)*12+1,year*12+1),end=slice.at(-1)!;years.push({year,stockValueCents:end.stockValueCents,homeValueCents:end.homeValueCents,mortgageBalanceCents:end.mortgageBalanceCents,equityCents:end.equityCents,saleProceedsCents:end.saleProceedsCents,principalCents:slice.reduce((s,m)=>s+m.principalCents,0),interestCents:slice.reduce((s,m)=>s+m.interestCents,0),ownerOutlayCents:slice.reduce((s,m)=>s+m.ownerOutlayCents,0),rentCents:slice.reduce((s,m)=>s+m.rentCents,0),stockContributionCents:slice.reduce((s,m)=>s+m.stockContributionCents,0)})}
  return {ok:true,result:{months,years,equityCrossovers:crossovers(months,m=>m.equityCents),saleCrossovers:crossovers(months,m=>m.saleProceedsCents)}};
}
