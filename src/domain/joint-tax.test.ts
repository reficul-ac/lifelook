...
describe("versioned rules and deductions",()=>{
  ...
  it("indexes future legal thresholds but keeps statutory thresholds fixed",()=>{const future=projectedTaxRules(2030,300);expect(future.federal.single.standardDeductionCents).toBe(18_100_00);expect(future.additionalMedicareThresholdCents["married-joint"]).toBe(250_000_00);expect(future.sources.every(x=>x.status==="projected")).toBe(true)});
