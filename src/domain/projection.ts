...
export const ProjectionEngine = {
  calculate(snapshot: FinancialSnapshot, scenario: Scenario): readonly AnnualProjection[] {
    ...
    let assetTotal = snapshot.assets.reduce((sum, a) => sum + a.valueCents, 0);
    let debt = snapshot.liabilities.reduce((sum, l) => sum + l.balanceCents, 0);
    ...
    for (let index = 0; index < scenario.horizon.months; index++) {
      ...
      assetTotal = grow(assetTotal, snapshot.assets.length ? snapshot.assets.reduce((s,a) => s + a.annualGrowthBps, 0) / snapshot.assets.length : 0, 1);
      for (const liability of snapshot.liabilities) debt = Math.max(0, debt - Math.min(debt, liability.minimumPaymentCents));
