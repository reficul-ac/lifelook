...
export function progressiveTax(amount:Cents,items:readonly TaxBracket[]):Cents{let tax=0,previous=0;for(const item of items){const ceiling=item.upToCents??amount;tax+=Math.round(Math.max(0,Math.min(amount,ceiling)-previous)*item.rateBps/10000);if(amount<=ceiling)break;previous=ceiling;}return tax;}
...
export function estimateTax(input:TaxabilityBreakdown,status:FilingStatus,pack:TaxRulePack,projected=false):TaxEstimate{
  ...
  const federalCents=progressiveTax(federalTaxable,fed.brackets),californiaCents=progressiveTax(caTaxable,ca.brackets);
...
export function estimateHouseholdTax(args:{year:number;status:FilingStatus;employees:readonly EmployeeWages[];nonWageTaxableCents?:Cents;deductions?:Partial<HouseholdDeductions>;thresholdInflationBps:number;futureCashFlowRatioBps?:number}):TaxLedger{
  ...
  const federalCents=progressiveTax(federalTaxable,pack.federal[status].brackets),californiaCents=progressiveTax(californiaTaxable,pack.california[status].brackets);
