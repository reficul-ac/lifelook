...
export interface Scenario { id: string; name: string; assumptions: GrowthAssumption; assumptionsInherited: boolean; events: readonly ScenarioEvent[]; allocations: readonly AllocationRule[]; withdrawals: readonly WithdrawalRule[]; horizon: ProjectionHorizon }
...
export interface ProjectionWarning { code: "account-depleted" | "unfunded-deficit" | "invalid-allocation" | "aggressive-assumption" | "negative-balance" | "payment-below-interest"; message: string; month: string; entityId?: string; inputField?: string }
