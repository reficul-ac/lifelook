...
export interface DebtPayoffGoal extends GoalCommon { type:"debt-payoff"; liabilityId:string }
...
export interface MajorPurchaseGoal extends GoalCommon { type:"major-purchase"; purchaseDate:string; costCents:Cents; destinationAccountId:string; mode:"expense"|"asset"; asset?:{id:string;name:string;downPaymentCents:Cents;acquisitionCostsCents:Cents;annualGrowthBps:BasisPoints;financing?:{liabilityId:string;name:string;principalCents:Cents;annualRateBps:BasisPoints;minimumPaymentCents:Cents};housingCosts?:HousingCosts} }
export interface RetirementGoal extends GoalCommon { type:"retirement"; participantIds:readonly string[]; retirementDates:Readonly<Record<string,string>>; planningThroughAges:Readonly<Record<string,number>>; desiredSpendingCents:Cents; healthcareCents:Cents; healthcareGrowthBps:BasisPoints; destinationAccountId:string; drawdownAccountIds:readonly string[]; pensions:readonly {id:string;name:string;monthlyCents:Cents;startDate:string;federalTaxableBps:BasisPoints;californiaTaxableBps:BasisPoints}[] }
export type ScenarioGoal = EmergencyFundGoal | DebtPayoffGoal | EducationGoal | MajorPurchaseGoal | RetirementGoal;
export interface GoalFundingResult { goalId:string; requiredCents:Cents; fundedCents:Cents; shortfallCents:Cents; earmarkedCents:Cents; targetCents:Cents; completionBps:BasisPoints; projectedCompletionDate?:string; targetResult:"on-track"|"completed"|"missed"|"infeasible" }
export type TaxableFlowType = "wages"|"pension"|"social-security"|"pretax-contribution"|"account-withdrawal";
export interface TaxableCashFlow { id:string; month:string; type:TaxableFlowType; amountCents:Cents; federalTaxableBps:BasisPoints; californiaTaxableBps:BasisPoints }
export interface ScenarioOverrides { accountReturnBps?:Readonly<Record<string,BasisPoints>>; recurring?:Readonly<Record<string,{annualGrowthBps?:BasisPoints;taxTreatment?:"none"|"pretax"}>>; assets?:Readonly<Record<string,{annualGrowthBps?:BasisPoints;housingCosts?:HousingCosts}>>; liabilities?:Readonly<Record<string,{annualRateBps?:BasisPoints;minimumPaymentCents?:Cents}>>; filingStatus?:FilingStatus; taxYear?:2025|2026; inflationBps?:BasisPoints; thresholdInflationBps?:BasisPoints; horizonMonths?:number }
...
export interface Scenario { id: string; name: string; assumptions: GrowthAssumption; assumptionsInherited: boolean; events: readonly ScenarioEvent[]; allocations: readonly AllocationRule[]; withdrawals: readonly WithdrawalRule[]; horizon: ProjectionHorizon; goals?:readonly ScenarioGoal[]; overrides?:ScenarioOverrides }
...
export interface ProjectionWarning { code: "account-depleted" | "unfunded-deficit" | "invalid-allocation" | "aggressive-assumption" | "negative-balance" | "payment-below-interest" | "goal-shortfall" | "goal-missed" | "goal-conflict" | "insufficient-earmark" | "retirement-depletion"; message: string; month: string; entityId?: string; inputField?: string }
